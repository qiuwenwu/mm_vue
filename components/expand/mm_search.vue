<template>
	<!-- 搜索栏 -->
	<div class="mm_search">
		<div class="reset" v-show="val"><text class="font_default" @click="clear()">取消</text></div><!-- /view -->
		<div class="search_warp">
			<div class="mm_icon"><text class='fa fa-search'></text></div><!-- /view -->
			<input type="text" :value="value" :placeholder="desc" @input="input_fun"></input>
		</div><!--/view-->
		<div class="search_ret lr" v-if="list.length > 0">
			<mm_list col="1" class="mini">
				<mm_item :url="o[vm.url]" v-for="(o, k) in list" :key="k">
					<mm_side>
						<mm_icon v-if="o[vm.icon]"></mm_icon>
						<div class="mm_icon" v-else><text class='fa fa-search'></text></div><!-- /view -->
					</mm_side>
