	...
	methods: {
		...
		get_obj(query) {
			...
			if (url) {
				...
				this.$get(this.toUrl(this.events('get_obj_before', this.query), url), function(json, status) {
					...
					if (obj) {
						_this.$obj.clear(_this.obj);
						_this.$obj.push(_this.obj, obj);
					}
				});
			}
		},
      ...
      this.list.clear();
		...
		upload(fun, func) {
			...
			this.uploading = 0;
			if (!fun) {
				fun = this.upload_after;
			}
			if (!func) {
				func = this.upload_main;
			}
			if (this.file.name) {
			...
			this.$upload(this.url_upload, this.form, fun, func);
		}
	},
	compuetd: {
		/// 分页数
