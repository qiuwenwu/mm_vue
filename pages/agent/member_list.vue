<template>
	<!-- 代理首页 -->
	<div class="page_agent" id="agent_member_list">
		<mm_bodyer>
			<mm_warp>
							...
							<mm_body class="lr">
								<mm_list col="1" class="big">
									<mm_item v-for="(o,k) in list" :key="k" @click.native="toPage(o.userId, o.people)">
										...
										<mm_main class="arrow">
											<mm_tip><div class="people">邀请人数 <text class="num">{{ o.people }}</text></div><!-- /view --><div class="count">持有礼包<text class="num">{{ $double(o.dbd + o.inactivated) }}</text></div><!-- /view --></mm_tip>
...
<script>
	...
	export default {
		...
		methods:{
			setImg(img){
				return img ? img : '/img/headImg.png'
			},
			toPage(id, p){
				if(p > 0){
