		...
		<mm_footer>
			<mm_warp>
				<mm_grid>
					<mm_col>
						<mm_group>
							<div class="buy_num"><input type="number" v-model="form.num" placeholder="请输入要购买的数量" placeholder-class="desc_color"></input></div><!-- /view -->
							<mm_btn type="default" class="buy_btn" @click.native="buy()">
								立即购买
							</mm_btn>
						</mm_group>
					</mm_col>
		...
		<mm_modal v-model="show">
			<mm_warp class="pay_warp">
				...
				<!-- <mm_item @click.native="pay('ali')">
										<mm_side>
											<mm_icon class="linear_blue" src="<i class='fa'>支</i>"></mm_icon>
										</mm_side>
										<mm_main class="arrow">
									...
									<mm_item @click.native="pay('wechat')">
										<mm_side>
											<mm_icon class="linear_success" src="<i class='fa fa-wechat'></i>"></mm_icon>
										</mm_side>
										<mm_main class="arrow">
									...
									<mm_item @click.native="pay('bank')">
										<mm_side>
											<mm_icon class="linear_purple" src="<i class='fa fa-credit-card'></i>"></mm_icon>
										</mm_side>
										<mm_main class="arrow">
									...
									<mm_item @click.native="pay('balance_cny')">
										<mm_side>
											<mm_icon class="linear_warning" src="<i class='fa fa-rmb'></i>"></mm_icon>
										</mm_side>
										<mm_main class="arrow">
											<mm_title>现金余额</mm_title>
											<mm_desc><text>{{ $double($num(user.achievement.canExtractMoney)) }}</text><text class="unit">元</text></mm_desc>
										</mm_main>
									</mm_item>
									<mm_item @click.native="pay('balance_btc')">
										...
										<mm_main class="arrow">
											...
											<mm_desc><text>{{ $float($num(user.assets.bitBalance)) }}</text><text class="unit">btc</text></mm_desc>
										</mm_main>
									</mm_item>
								</mm_list>
							</mm_body>
	...
	export default {
		...
		methods: {
			...
			submit_after(json, status) {
				if (json) {
					if (json.data) {
						var url = '/pages/pay/' + this.way + '?id=' + json.data.id;
						uni.navigateTo({
							url: url
						});
						return;
			...
			get_dbd_statement() {
				...
				this.$get('~/paper/title?title=礼包法律', function(json, status) {
					if (json) {
						var lt = json.data;
						if (lt.length > 0) {
