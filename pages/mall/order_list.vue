					...
					<mm_col class="pn-t">
						<mm_block>
							<mm_head class="font_small">
								<mm_title>订单数：{{ count_num }}</mm_title>
							</mm_head>
						</mm_block>
					</mm_col>
						...
						<mm_block class="dbd">
								...
								<mm_list col="1" class="dbd_item">
									...
									<mm_item class="big">
										...
										<mm_main class="introduce">
											<mm_title>{{ dbd.description }}<text class="count">{{ o.amount }}</text></mm_title>
							...
							<mm_foot class="bt">
								<mm_group class="small">
									<mm_btn v-if="o.state == 2" @click.native="cancel(o.id)">取消订单</mm_btn>
									<navigator class="mm_btn_info-x" :url="'./order_view?id=' + o.id" v-else>查看详情</navigator>
									<mm_btn type="default-x" v-if="o.state == 2 || o.state == 3" @click.native="pay_way(o.payType, o.id)">继续支付</mm_btn>
								</mm_group>
							</mm_foot>
						</mm_block>
					</mm_col>
	...
	export default {
		...
		methods: {
			cancel(id) {
				...
				uni.showModal({
					title: '取消订单',
					content: '订单取消后将不能恢复，可能错失购买机会，是否确定要取消？',
					success: function(res) {
						if (res.confirm) {
							_this.$post('~/pay/close', {
								orderId: id
			...
			get_list_after(json, status) {
				...
				if (json) {
					var d = json.data;
					if (d) {
						this.count_num = d.totalElements;
